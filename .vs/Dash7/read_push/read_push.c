...
#define CUSTOM_MESSAGE_SIZE 1 // Expected size of CustomMessage
...
void sendHi(){
    ...
    uart_send_string(uart_handle, "Hey dit is mijn testbericht\r\n");
        // reschedule sensor measurement  
}

void sendTask(){
    sendHi();
    timer_post_task(&sendTask, SENSOR_INTERVAL_SEC);
}
...
void uart_rx_callback(uart_handle_t* uart_handler, uint8_t byte) {
    ...
    if (transmissionStarted) {
      if (data_buffer_index < DATA_BUFFER_SIZE) {
		  ...
		  uart_send_string(uart_handle, "Success: transmission started\r\n");
          if (byte == END_DELIMITER) {
			  ...
			  uart_send_string(uart_handle, "Success, transmission ended\r\n");
          }
      } else {
...
void bootstrap() {
    ...
    sendTask();
    sched_register_task(&sendTask);
    //sched_post_task(&sendHi);
